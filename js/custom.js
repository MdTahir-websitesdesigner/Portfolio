    ...
    jQuery(document).ready(function($) {
        ...
        function initSkillBars() {
            ...
            function animateBar(bar) {
                ...
                var progressAttr = bar.querySelector('.item-progress');
                var percent = parseInt(bar.getAttribute('data-percent') || (progressAttr && progressAttr.getAttribute('data-percent')) || 0, 10);
                percent = Math.max(0, Math.min(100, percent));
                var visualPct = Math.max(5, Math.min(95, percent));
                bar.style.setProperty('--pct', visualPct + '%');
                // add accessible label with skill name
                var skillEl = bar.parentElement.querySelector('.skill-text');
                var skillName = skillEl && skillEl.textContent ? skillEl.textContent.trim() : 'Skill';
                bar.setAttribute('aria-label', skillName + ' ' + percent + '%');
                var progress = bar.querySelector('.item-progress');
                if (progress) {
                    progress.style.width = percent + '%';
                    progress.setAttribute('aria-valuenow', String(percent));
                    // update container role as well
                    bar.setAttribute('aria-valuenow', String(percent));
                }
                var percentElem = bar.querySelector('.percent .percent-value');
                var percentBubble = bar.querySelector('.percent');
                if (percentElem) {
                    var startTime = null;
                    var duration = 800;
                    ...
                    function step(ts) {
                        ...
                        percentElem.textContent = Math.floor(t * percent) + '%';
                        if (t < 1) requestAnimationFrame(step);
                        else {
                            percentElem.textContent = percent + '%';
                            // subtle pop on completion (respect reduce-motion)
                            if (!prefersReduced && percentBubble) {
                                percentBubble.classList.add('pct-pop');
                                setTimeout(function() { percentBubble.classList.remove('pct-pop'); }, 800);
                            }
                        }
                    }
                    requestAnimationFrame(step);
                } else {
                    // fallback: still add a brief pop if no animated counter
                    if (!prefersReduced && percentBubble) {
                        percentBubble.classList.add('pct-pop');
                        setTimeout(function() { percentBubble.classList.remove('pct-pop'); }, 800);
                    }
            ...
            // allow keyboard focus to trigger the animation
            bars.forEach(function(b) {
                b.addEventListener('focus', function() { animateBar(b); });
                b.addEventListener('keydown', function(e) { if (e.key === ' ' || e.key === 'Enter') { e.preventDefault();
                        animateBar(b); } });
            });
